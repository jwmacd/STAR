...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        **kwargs,
        ...
        # print('use Spatial-Temporal LIEM')
        local_fea = rearrange(img_attention_input, 'b (t h w) c -> b c t h w', h=self.height, w=self.width)
        local_fea = layer.liem(local_fea)

        img_attention_input = rearrange(local_fea, 'b c t h w -> b (t h w) c')
